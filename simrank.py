...
next(csv_reader)
logs_tuple = [ tuple(log[0:2]) for log in csv_reader ]
# print(logs_tuple)
queries = list(set([ log[0] for log in logs_tuple ]))
ads = list(set([ log[1] for log in logs_tuple ]))   # = item
...
# print(query_simrank('i75','i40',0.8))
        ...
        with open('sim.csv','w') as cf:
            ...
            for qi in queries:
                for qj in queries:
                    ...
                    if i < j:
                        sim = query_simrank(qi, qj, C)
                        if sim>0:
                            writer.writerow([qi, qj, sim])
